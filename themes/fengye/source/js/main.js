...
function registerLanguageToggle() {
  ...
  function applyLanguage(lang) {
    ...
    document.querySelectorAll("[data-i18n-lang]").forEach((item) => {
      item.hidden = item.dataset.i18nLang !== lang;
    });
  ...
  if (toggle) {
    toggle.addEventListener("click", () => {
      const current = document.documentElement.dataset.siteLang || getDefaultLanguage();
