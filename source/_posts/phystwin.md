...
摘要：为真实物体创建物理数字孪生体在机器人学、内容创作及扩展现实领域具有巨大潜力。PhysTwin 通过动态物体在交互过程中的稀疏视频，生成具备照片级及物理真实感、可实时交互的虚拟复制体。其核心包含两大创新：(1) 基于物理定律的表征方式，融合弹簧-质量模型实现真实物理模拟、生成式几何模型构建形状结构、高斯溅射技术实现渲染；(2) 多阶段优化逆向建模框架，从视频中重构完整几何结构、推断致密物理属性并复现真实外观。项目主页：https://jianghanxiao.github.io/phystwin-web/
...
进行位置和速度的更新时，文章采取半隐式欧拉法，即对任意节点$i$：
...
![PhysTwin overview](/images/notes/Pasted%20image%2020250807161758.png)
...
$$
其中$C_\mathrm{geometry}$为推测的点云$\hat X_t$与观测点云$X_t$之间的 Chamfer Distance，$C_\mathrm{motion}$是预测点$\hat x_t$与观测点$x_t$之间的轨迹误差。其中观测轨迹通过CoTracker3获取2维轨迹，随后通过深度图投影到3维空间中。

**Q：推测点云$\hat X_t$与观测点云$X_t$并不是一一对应的，该如何找到预测点$x_t$对应的观测点$x_t$？**
...
为从视频中获取控制点，文章利用Grounded-SAM2得到手部mask，并用CoTracker3得到手部运动轨迹，将控制点提升到3D空间后，采用 FPS（farthest-point sampling）采样获得最终的控制点集合。
...
![Sparse-to-dense optimization](/images/notes/Pasted%20image%2020250811175123.png)
