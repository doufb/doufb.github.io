...
摘要：基于视觉观测重建和模拟弹性物体对于计算机视觉与机器人技术应用至关重要。现有方法（如3D高斯模型）虽能建模三维外观与几何形态，但无法估算物体物理属性并进行动态模拟。核心挑战在于如何整合兼具表现力与效率的物理动力学模型。Spring-Gaus 是一种基于多视角视频的弹性物体三维物理表征方法，通过将3D弹簧质点模型与3D高斯核相结合，实现了物体视觉外观、几何形态及物理动态特性的联合重建。该方法支持在不同初始状态与环境属性下进行未来状态预测与动态模拟。项目页面：https://zlicheng.com/spring_gaus。
# Overview

![Spring-Gaus overview](/images/notes/Pasted%20image%2020250804194224.png)

![Spring-Gaus pipeline](/images/notes/Pasted%20image%2020250804171711.png)
...
# 弹簧-质点模型
由于静态重建部分与原始3DGS方法没有区别，在此不再赘述，我们直接进入弹簧-质点模型的介绍。
## 构建锚点
...
![Soft vector ablation](/images/notes/Pasted%20image%2020250806115600.png)

从文章给出的消融实验来看，软向量确实能明显改善仿真结果。
# 优化
## 减少参数量
...
![Static-dynamic scene calibration](/images/notes/Pasted%20image%2020250806162006.png)
